 ...
 */
namespace Malocher\Test\Coverage\Cqrs\Bus;
 ...
 * @package Malocher\Test\Coverage\Cqrs\Bus
 */
class SystemBusTest extends AbstractBusTest
{
    ...
    public function testClosureInvokeCommand()
    {
        ...
        $this->bus->mapCommand('Malocher\Test\Coverage\Mock\Command\MockCommand', function (MockCommand $command) {
            $command->edit();
        });
        ...
        $this->assertEquals('Malocher\Test\Coverage\Mock\Command\MockCommand', $this->invokeCommandCommand->getMessageClass());
        $this->assertEquals('Malocher\Test\Coverage\Mock\Command\MockCommand', $this->commandInvokedEvent->getMessageClass());
    }
    ...
    public function testClosureExecuteQuery()
    {
        ...
        $gate->enableSystemBus();
        $gate->attach($this->bus);
        $this->bus->mapQuery('Malocher\Test\Coverage\Mock\Query\MockQuery', function (MockQuery $query) {
            $query->edit();
        });
        ...
        $this->assertEquals('Malocher\Test\Coverage\Mock\Query\MockQuery', $this->executeQueryCommand->getMessageClass());
        $this->assertEquals('Malocher\Test\Coverage\Mock\Query\MockQuery', $this->queryExecutedEvent->getMessageClass());
    }
    ...
    public function testClosurePublishEvent()
    {
        ...
        $gate->enableSystemBus();
        $gate->attach($this->bus);
        $this->bus->registerEventListener('Malocher\Test\Coverage\Mock\Event\MockEvent', function (MockEvent $event) {
            $event->edit();
        });
        ...
        $this->assertEquals('Malocher\Test\Coverage\Mock\Event\MockEvent', $this->publishEventCommand->getMessageClass());
        $this->assertEquals('Malocher\Test\Coverage\Mock\Event\MockEvent', $this->eventPublishedEvent->getMessageClass());
    }
    ...
    public function testArrayMapExecuteQuery()
    {
        ...
        $gate->attach($this->bus);
        $this->bus->mapQuery(
            'Malocher\Test\Coverage\Mock\Query\MockQuery',
            array('alias' => 'Malocher\Test\Coverage\Mock\Query\MockQueryHandler', 'method' => 'handleQuery')
        );
        $gate->getSystemBus()->mapCommand(
            'Malocher\Cqrs\Command\ExecuteQueryCommand',
            array('alias' => 'Malocher\Test\Coverage\Mock\Command\MockCommandHandler', 'method' => 'handleCommand')
        );
        $gate->getSystemBus()->registerEventListener(
            'Malocher\Cqrs\Event\QueryExecutedEvent',
            array('alias' => 'Malocher\Test\Coverage\Mock\Event\MockEventHandler', 'method' => 'handleEvent')
        );
        $mockQuery = new MockQuery();
    ...
    public function testArrayMapInvokeCommand()
    {
        ...
        $gate->attach($this->bus);
        $this->bus->mapCommand(
            'Malocher\Test\Coverage\Mock\Command\MockCommand',
            array('alias' => 'Malocher\Test\Coverage\Mock\Command\MockCommandHandler', 'method' => 'handleCommand')
        );
        $gate->getSystemBus()->mapCommand(
    ...
    public function testArrayMapPublishEvent()
    {
        ...
        $this->bus->registerEventListener(
            'Malocher\Test\Coverage\Mock\Event\MockEvent',
            array('alias' => 'Malocher\Test\Coverage\Mock\Event\MockEventHandler', 'method' => 'handleEvent')
        );
        $gate->getSystemBus()->mapCommand(
    ...
    public function testArrayMapExecuteQueryMissingAdapterTrait()
    {
        ...
        $gate->getSystemBus()->mapCommand(
            'Malocher\Cqrs\Command\ExecuteQueryCommand',
            array('alias' => 'Malocher\Test\Coverage\Mock\Command\MockCommandHandlerNoAdapter', 'method' => 'handleCommand')
        );
        $gate->getSystemBus()->registerEventListener(
            'Malocher\Cqrs\Event\QueryExecutedEvent',
            array('alias' => 'Malocher\Test\Coverage\Mock\Event\MockEventHandler', 'method' => 'handleEvent')
    ...
    public function testArrayMapPublishEventMissingAdapterTrait()
    {
        ...
        $gate->getSystemBus()->registerEventListener(
            'Malocher\Cqrs\Event\EventPublishedEvent',
            array('alias' => 'Malocher\Test\Coverage\Mock\Event\MockEventHandlerNoAdapter', 'method' => 'handleEvent')
        );
        $mockEvent = new MockEvent();
        $gate->getBus($this->bus->getName())->publishEvent($mockEvent);
    }
